                        ...
                        <div key={entry.id} className="card-hover group flex flex-col h-full">
                            ...
                            <div className="mt-auto flex flex-col gap-3">
                                ...
                                <div className="flex items-center justify-end gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity pt-3 border-t border-gray-100 dark:border-gray-800">
                                    {isOwner && <button onClick={() => handleEditEntry(entry.id)} className="btn-ghost p-1.5" title={t('vault.edit')}><Pencil className="w-4 h-4" /></button>}
                                    ...
                                    {isOwner && <button onClick={() => handleDeleteEntry(entry.id)} className="btn-ghost p-1.5 text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300" title={t('vault.delete')}><Trash2 className="w-4 h-4" /></button>}
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {groups.map(group => (
                        <div
                            key={group.id}
                            className="card-hover cursor-pointer group flex flex-col h-full relative"
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-1 min-w-0 pr-6">
                                    ...
                                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                        ...
                                        <span className="mx-2 block sm:inline">·</span>
                                        <span className="text-gray-400 dark:text-gray-500 block sm:inline">{t('groups.owner')}: {group.owner_username}</span>
                                    </p>
                                </div>
                                {group.owner_id === currentUserId && (
                                    <button onClick={e => { e.stopPropagation(); handleDeleteGroup(group.id); }} className="absolute top-4 right-4 text-red-400 hover:text-red-600 dark:hover:text-red-300 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity bg-white dark:bg-gray-900 rounded-full p-1" title={t('groups.delete_group')}>
